...
raise "install smoke does not cache managed tools" unless install_text.include?("managed-tools-v2-")
raise "managed tool builds do not use runner CPU capacity" if managed_tools.include?("make -j2")
raise "managed tool binary cache is missing" unless managed_tools.include?('cache_root="$MANAGED_TOOLS_CACHE_DIR/v2/$TARGET/$cache_variant"')
raise "managed tool cache must copy only managed binaries" unless managed_tools.include?('for managed_binary in tmux wg wg-quick; do')
raise "release package must isolate managed tool output" unless unix_packager.include?('managed_tools_work="$work/.managed-tools"')
raise "release package must validate every binary version" unless unix_packager.include?('grep -F -- "$VERSION"')
